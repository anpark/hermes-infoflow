...
| `test_prompt_behavior_glm.py` | Live GLM prompt-behavior gate for `watch_mention` / `watch_regex`: verifies selected prompts read history, use relevant skills/tools, or emit `NO_REPLY` as expected. Uses `scripts/sim/prompt_behavior_cases.json` and the configured Hermes main model. |
